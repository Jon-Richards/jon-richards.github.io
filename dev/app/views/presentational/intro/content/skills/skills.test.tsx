...
describe('The Skills component.', () => {
  it('Should match the snapshot.', () => {
    const test_1 = shallow(<Skills skills={mockSkills} />);
  ...
  it('Should render according to its props.', () => {
    const test_1 = shallow(<Skills skills={mockSkills} />);
    const test_2 = shallow(<Skills skills={[]} />);

    expect(
      test_1.find('[data-testid="intro__content__skills__list"]').children()
        .length
    ).toBe(3);
    expect(
      test_2.find('[data-testid="intro__content__skills__list"]').children()
        .length
    ).toBe(0);
  });
});
