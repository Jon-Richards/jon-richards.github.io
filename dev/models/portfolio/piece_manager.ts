...
export class PieceManager {
  ...
  constructor(
    ...
    if (Array.isArray(pieces)) {
      ...
      if (process.env.NODE_ENV !== 'test') {
        console.error(
          'ERROR: Parameter "pieces" must be an array of Piece instances.'
        );
      }
      this.pieces = [];
    }
  ...
  /**
   * Checks an array of Pieces against a series of validators and removes any
   * that do not pass.
  ...
  /**
   * Maps an array of raw Piece objects into an array of Piece instances so that
  ...
  /**
   * Checks an array of Pieces for empty properties and removes the piece if any
   * are found.
  ...
  private throwEmptyPropertyError(property: string, uuid: Piece['uuid']): void {
    if (process.env.NODE_ENV !== 'test') {
      console.error(
        `ERROR: Property: ${property} is missing from portfolio piece with ` +
          `uuid: ${uuid}. The piece will be discarded.`
      );
    }
  }

  /**
   * Checks an array of Pieces for duplicate information and removes the
   * duplicate Piece if one is found.
   * @param pieces An array of Pieces.
  ...
  private removeDuplicatePieces(pieces: PieceShape[]): PieceShape[] {
    ...
    pieces.forEach(piece => {
      ...
      checkedPieces.forEach(checkedPiece => {
        hasDuplicateId = this.compareProperty(
          piece.uuid,
          checkedPiece.uuid,
          'uuid'
        );
        hasDuplicateUrl = this.compareProperty(
          piece.url,
          checkedPiece.url,
          'url'
        );
        hasDuplicateTitle = this.compareProperty(
          piece.title,
          checkedPiece.title,
          'title'
        );
  ...
  /**
   * Compares a given property against another.  Raises an error if the
  ...
  private compareProperty<T>(propA: T, propB: T, propName: string): boolean {
    ...
    if (areEqual && process.env.NODE_ENV !== 'test') {
      console.error(
        `ERROR: Found duplicate piece with property: ${propName}.  ` +
          `The duplicate will be discarded.`
      );
    }
    return areEqual;
  }
